...
Properties describe *what an object is right now*: its width, its colour, its text. Signals describe *what just happened to it*: a button was clicked, an animation finished, a value changed.
...
```admonish note "A handler is a normal property"
A handler is written like any other property, and follows the same rules. Its value can be a single expression, as above, or a whole block of JavaScript:
...
Here is a complete example using one of each. It also introduces a **MouseArea**: an invisible item that covers a region of the window and reports what the mouse does over it. It is the way to make anything in QML react to the mouse, and it emits a `clicked` signal of its own, carrying a `mouse` parameter that holds the position of the click. We will come back to it thoroughly in a later chapter, so for now just take it as a rectangle that tells you where it was clicked.
...
Here is the part that makes QML a powerfull language: **every property automatically comes with its own signal**, emitted whenever its value changes. The signal is named after the property, followed by `Changed`, so the handler for a property `counter` is `onCounterChanged`.
...
The base use case of **Connections** involve using theses properties:
...
The old form, written like a normal handler with a colon, is still accepted for backwards compatibility, but the Qt documentation states plainly that it is *"not recommended"*:
...
For completeness: signals are also objects, and they carry a `connect()` method. It lets you wire things up at runtime, from inside a function:
...
Signals are much easier to feel than to read about, and `console.log()` makes them visible. It is again a good time to experiment a bit. Here is some things you can try: put a **Button** in a window and log its `clicked`, `pressed` and `released` signals to see in which order they arrive. Add a `property int counter` with an `onCounterChanged` handler, increment it from the button, and check that assigning the same value twice in a row only logs once. Declare a signal of your own carrying a string, emit it from the button with different values, and log it from a **Connections** instead of a handler.
```
